...
- There are now two distinct variants of the mask decoder, one for images ('interactivity') and one for video segmentation. These have very similar structure, but completely separate weights!
...
#### Coordinate encoder
- There are now two copies of the coordinate encoder, one for each mask decoder variant (with separate weights)
...
- Now expects mask inputs to have a shape of 1xMxHxW (M multiplex count, H & W are height and width)
- Also expects object score input to have a shape of Mx1
...
- Zero padding of masks is now included if fewer than 16 masks are provided (this seems like a strange choice)
...
- Each input has it's own q/k/v projection (there are now 2 q- & k-projections)
