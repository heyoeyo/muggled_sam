...
class SAMV3p1CoordinateEncoder(nn.Module):
    """
    ...
    The code here is adapted from the original segment-anything repos:
    ...
    https://github.com/facebookresearch/sam3/blob/757bbb0206a0b68bee81b17d7eb4877177025b2f/sam3/sam/prompt_encoder.py#L200

    Performs positional encoding of (x,y) coordinates, used to encode both foreground & background
    prompt points, top-left & bottom-right bounding box coordinates as well as grid cell positions
    (i.e. every image patch token position).

    More specifically, this model converts (x,y) coordinates into larger vectors, which are sized
    to match the image patch tokens from the image encoder. For example, a single (x,y) pair,
    like (0.5, 0.5) will be transformed into a 256-dimensional (by default) vector. This allows
    the coordinates to be used within a transformer model (i.e. for cross-attention with image tokens).
