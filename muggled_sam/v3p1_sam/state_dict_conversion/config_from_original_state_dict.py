...
def get_model_config_from_state_dict(state_dict):
    ...
    hardcoded_memimgfusion_num_heads = 8
...
def get_features_per_prompt_token(state_dict):
    ...
    target_key = "tracker.model.interactive_sam_prompt_encoder.not_a_point_embed.weight"
...
def get_num_output_mask_tokens(state_dict, multiplex_channels: int = 16):
    ...
    assert (
        target_key_1 in state_dict.keys()
    ), f"Error determining the number of output mask tokens! Couldn't find key: {target_key_1}"

    target_key_2 = "tracker.model.sam_mask_decoder.mask_tokens.weight"
    assert (
        target_key_2 in state_dict.keys()
    ), f"Error determining the number of output mask tokens! Couldn't find key: {target_key_2}"
...
def get_mask_decoder_features_per_token(state_dict):
    ...
    target_key = (
        "tracker.model.interactive_sam_mask_decoder.transformer.layers.0.cross_attn_image_to_token.q_proj.weight"
    )
    assert (
        target_key in state_dict.keys()
...
def get_mask_decoder_block_count(state_dict):
    num_layers = _find_num_seq_elems(state_dict, "tracker.model.sam_mask_decoder.transformer.layers")
...
def get_features_per_memory_token(state_dict):
    ...
    target_key = "tracker.model.maskmem_backbone.pix_feat_proj.weight"
...
def get_memory_encoder_downsample_layer_count(state_dict):
    ...
    This one is a bit weird. The layers look like:
        ...
        'tracker.model.maskmem_backbone.mask_downsampler.encoder.0.bias'
        'tracker.model.maskmem_backbone.mask_downsampler.encoder.1.weight'
        'tracker.model.maskmem_backbone.mask_downsampler.encoder.1.bias'
        ...
        'tracker.model.maskmem_backbone.mask_downsampler.encoder.12.bias'
        etc.
    ...
    # Get indexing of entry with the target prefix
    is_target_key = lambda key: key.startswith("tracker.model.maskmem_backbone.mask_downsampler.encoder")
...
def get_memory_encoder_mixer_layer_count(state_dict):
    num_layers = _find_num_seq_elems(state_dict, "tracker.model.maskmem_backbone.fuser.layers")
...
def get_memory_image_fusion_layer_count(state_dict):
    num_layers = _find_num_seq_elems(state_dict, "tracker.model.transformer.encoder.layers")
