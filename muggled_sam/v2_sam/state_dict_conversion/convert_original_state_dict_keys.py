...
def convert_state_dict_keys(
...
) -> tuple[dict[SAM2ModuleType, dict], dict[str, str]]:
    """
    ...
    for orig_key, orig_data in original_state_dict.items():
        ...
        if found_key(new_key):
            ...
            _update_sd(SAM2ModuleType.image_encoder, orig_key, new_key, mod_data)
            continue

        new_key = _convert_coordencoder_keys(orig_key)
        if found_key(new_key):
            _update_sd(SAM2ModuleType.coordinate_encoder, orig_key, new_key, orig_data)
            continue

        new_key = _convert_promptencoder_keys(orig_key)
        if found_key(new_key):
            _update_sd(SAM2ModuleType.prompt_encoder, orig_key, new_key, orig_data)
            continue
        ...
        if found_key(new_key):
            ...
            _update_sd(SAM2ModuleType.memory_encoder, orig_key, new_key, mod_data)
            continue
