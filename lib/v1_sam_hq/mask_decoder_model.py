...
class SAMHQV1MaskDecoder(nn.Module):
    """ """
    ...
    def __init__(
        ...
        # Set up SAM-HQ token
        self.cls_hq_token = nn.Parameter(torch.empty(1, input_channels))
    ...
    def forward(
        ...
        # For clarity
        final_img_tokens_bchw, first_img_tokens_bchw = encoded_image_bchw
        ...
        if blank_promptless_output and (num_prompts == 0) and (mask_hint is None):
            return self.maskgen.make_blank_results(final_img_tokens_bchw, batch_size_prompts)

        # Concatenate learned 'cls' tokens to prompts
        cls_tokens = torch.cat([self.cls_iou_token, self.cls_hq_token, self.cls_mask_tokens], dim=0).unsqueeze(0)
        ...
        img_tokens_bchw = self.maskhint_encoder(final_img_tokens_bchw, mask_hint)
        ...
        hq_token_out = encoded_cls_tokens[:, 1, :]
        mask_tokens_out = encoded_cls_tokens[:, 2:, :]

        # Produce final output mask & quality predictions
        mask_preds = self.maskgen(img_tokens_bchw, mask_tokens_out, first_img_tokens_bchw, hq_token_out)
...
class MaskGen(nn.Module):
    ...
    def __init__(self, input_channels, num_mask_tokens):
        ...
        # Extra MLP specific to SAM-HQ
        self.hq_token_mlp = MLP3Layers(input_channels, upscaler_channels)
        self.embedding_maskfeature = nn.Sequential(
            nn.Conv2d(upscaler_channels, hidden_channels, 3, 1, 1),
            LayerNorm2d(hidden_channels),
            nn.GELU(),
            nn.Conv2d(hidden_channels, upscaler_channels, 3, 1, 1),
        )
    ...
    def forward(
        self, image_tokens_bchw: Tensor, cls_mask_tokens: Tensor, early_image_tokens_bchw: Tensor, cls_hq_token: Tensor
    ) -> Tensor:
        ...
        # Do SAM-HQ work
        early_img_tokens = self.embedding_maskfeature(upscaled_img_tokens) + early_image_tokens_bchw
        encoded_hq_token = self.hq_token_mlp(cls_hq_token).unsqueeze(1)
        mask_hq_pred = torch.einsum("bnc, bchw -> bnhw", encoded_hq_token, early_img_tokens)

        # Take dot product (along channel dimension) of cls tokens with image patches for final masks
        mask_preds = torch.einsum("bnc, bchw -> bnhw", encoded_mask_tokens, upscaled_img_tokens)

        return mask_preds + mask_hq_pred
