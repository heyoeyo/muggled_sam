...
def convert_state_dict_keys(config_dict: dict, original_state_dict: dict) -> dict[str, dict]:
    ...
    for orig_key, orig_data in original_state_dict.items():
        ...
        if found_key(new_key):
            ...
            # Correct layernorm2d weight shapes for SAM-HQ
            layernorm_key_hints = ("compress_vit_feat.1", "embedding_encoder.1")
            mod_data = _reshape_layernorm2d(new_key, mod_data, *layernorm_key_hints)
        ...
        if found_key(new_key):
            ...
            # Correct layernorm2d weight shapes for SAM-HQ
            layernorm_key_hints = ("embedding_maskfeature.1",)
            mod_data = _reshape_layernorm2d(new_key, mod_data, *layernorm_key_hints)
...
def _convert_imgenc_keys(key: str, blocks_per_stage: int) -> None | str:
    ...
    # For SAM-HQ: Move image embedding weights from mask decoder over to the image encoder
    if key.startswith("mask_decoder.embedding_encoder"):
        return key.replace("mask_decoder.", "")
    if key.startswith("mask_decoder.compress_vit_feat"):
        return key.replace("mask_decoder.", "")
...
def _convert_maskdecoder_keys(key: str) -> None | str:
    ...
    # Specific to SAM-HQ
    if key.startswith("mask_decoder.hf_token"):
        return "cls_hq_token"
    ...
    # Special handling of SAM-HQ specific 'mask token mlp', which is like a 5th mask mlp
    if key.startswith("mask_decoder.hf_mlp"):
        layer_idx = get_nth_integer(key, 0)
        new_idx = 2 * layer_idx
        weight_or_bias = get_suffix_terms(key, 1)
        return f"maskgen.hq_token_mlp.layers.{new_idx}.{weight_or_bias}"

    # Extra encoder model for SAM-HQ
    if key.startswith("mask_decoder.embedding_maskfeature"):
        return key.replace("mask_decoder", "maskgen")
