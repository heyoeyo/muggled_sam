...
from muggled_sam.demo_helpers.model_info import get_preencoding_hw
...
if max_side_length is None:
    ...
    preencode_hw = get_preencoding_hw(detect_model, prep_img, max_side_length, use_square_sizing)
...
for _ in range(num_warmup_iterations):
    encoded_img = detect_model.encode_image(test_img, **imgenc_config_dict)
    encoded_exemplars = detect_model.encode_exemplars(encoded_img, **prompts_dict)
    mask_preds, box_preds, detection_scores, presence_score = detect_model.generate_detections(
        encoded_img, encoded_exemplars
    )
if torch.cuda.is_available():
...
for _ in range(num_detection_generation_iterations):
    mask_preds, _, _, _ = detect_model.generate_detections(encoded_img, encoded_exemplars)
if torch.cuda.is_available():
    torch.cuda.synchronize()
t2 = perf_counter()
total_time_ms = 1000 * (t2 - t1)
