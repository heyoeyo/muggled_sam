...
As with the image script, you can add `--help` to the end of this command to see a list of additional flags. For example, you can add the flag `--use_webcam` to run segmentation on a live webcam feed. Using `-b 512` to reduce the processing resolution can provide a significant speed up if needed (box prompting works better at reduced resolutions btw!). The per-frame segmentation results can be saved as .pngs, to convert these into a video file please see the [video from frames](.readme_assets/video_from_frames.md) explainer.
